    ...
    for (i = 0, iLen = oSettings.aoData.length ; i < iLen ; i++) {
        ...
        if (iColumn === undefined) {
            for (j = 0, jLen = columns.length ; j < jLen ; j++) {
                ...
                if (val.toUpperCase().indexOf(sSearch) >= 0) {
                    aOut.push(oSettings.aoData[i].nTr);
                }
            }
        }
        else if (this.fnGetData(i, iColumn).toUpperCase().indexOf(sSearch) >= 0) {
            var descrip=this.fnGetData(i, iColumn).toUpperCase();
            var elements = descrip.split(sSearch);
            if (valor == 1 && elements[0] == "")
                aOut.push(oSettings.aoData[i].nTr);
            else {
                if (valor == 3 && elements[elements.length - 1] == "")
                    aOut.push(oSettings.aoData[i].nTr);
                else
                {
                    if(valor==2)
